...
import { Check, ChevronDown, Minus, Play, Plus } from "lucide-react";
...
const AccordionItem = ({ title, children, isOpen, onClick }: any) => {
	return (
		<div className="border-b border-black/10">
			<button className="flex w-full items-center justify-between py-5 text-left font-medium text-[#121212] transition-colors hover:text-[#C9A86A] focus:outline-none" onClick={onClick}>
			...
			<AnimatePresence initial={false}>
				{isOpen && (
					<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="overflow-hidden">
						<div className="pb-6 text-sm text-gray-500 leading-relaxed">{children}</div>
					</motion.div>
				)}
			</AnimatePresence>
		</div>
	...
	return (
		<div className="bg-[#F5F5F2] min-h-screen text-[#121212] overflow-x-hidden pt-24">
			...
			<div className="fixed inset-0 w-full h-full pointer-events-none select-none flex justify-center items-center z-0 overflow-hidden">
				<span className="text-[25vw] font-serif font-bold italic text-transparent tracking-tighter whitespace-nowrap opacity-5" style={{ WebkitTextStroke: "2px #121212" }}>
					ZODIAC
				</span>
			</div>
			...
			<div className="max-w-[1440px] mx-auto px-6 md:px-12 py-4 md:py-8">
				...
				<div className="flex items-center gap-3 mb-6 text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase">
					<Link to="/" className="text-[#121212] hover:text-[#C9A86A] transition-colors">
						Zodiac
					</Link>
					<Play size={8} className="text-[#C9A86A] fill-[#C9A86A]" />
					<Link to="/shop" className="text-[#121212] hover:text-[#C9A86A] transition-colors">
						Shop
					</Link>
					<Play size={8} className="text-[#C9A86A] fill-[#C9A86A]" />
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
					...
					<div className="flex flex-col order-2 lg:order-1">
						<motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
							...
							<p className="text-[#121212]/70 leading-relaxed mb-10 text-lg max-w-lg mt-6">{product.description}</p>
							...
							<div className="space-y-6 mb-12">
								<div className="flex items-start gap-4">
									...
									<div>
										...
										<p className="text-[#121212]/60 text-sm">
											{product.effect} - {product.strength} Strength
										</p>
									</div>
								</div>
								<div className="flex items-start gap-4">
							...
							<div className="flex flex-col sm:flex-row items-center gap-6 mb-12">
								<div className="text-3xl font-serif font-bold text-[#121212] min-w-[100px]">${product.price}</div>

								<div className="flex items-center border border-black/10 rounded-sm bg-white h-14">
									<button className="px-4 h-full hover:bg-gray-50 transition-colors text-[#121212]" onClick={() => setQuantity(Math.max(1, quantity - 1))}>
										<Minus size={16} />
									</button>
									<span className="w-12 text-center text-sm font-medium text-[#121212]">{quantity}</span>
									<button className="px-4 h-full hover:bg-gray-50 transition-colors text-[#121212]" onClick={() => setQuantity(quantity + 1)}>
										<Plus size={16} />
									</button>
								</div>
							...
							<div className="border-t border-black/10 pt-2 w-full max-w-lg">
								<AccordionItem title="Full Details" isOpen={openSection === "usage"} onClick={() => setOpenSection(openSection === "usage" ? null : "usage")}>
									{product.usage}
								...
								<AccordionItem title="All Ingredients" isOpen={openSection === "ingredients"} onClick={() => setOpenSection(openSection === "ingredients" ? null : "ingredients")}>
					...
					<motion.div initial={{ opacity: 0, scale: 0.95, x: 20 }} animate={{ opacity: 1, scale: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative order-1 lg:order-2 flex justify-center items-center">
						...
						<div className="relative w-full max-w-lg lg:max-w-xl aspect-[4/5] lg:aspect-square flex items-center justify-center">
							...
							<img src={explodedView} alt="Product Exploded View" className="w-full h-auto object-contain relative z-10 drop-shadow-xl mix-blend-multiply" />
						</div>
					</motion.div>
				</div>
			</div>
