			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-24">
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
					<div className="lg:col-span-2 space-y-6">
						<AnimatePresence>
							{cart.map((item) => (
								<motion.div key={item.id} layout initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex gap-6 border-b border-black/10 pb-6">
									...
									<div className="flex-1 flex flex-col justify-between">
										...
										<div className="flex justify-between items-center mt-4">
											<div className="flex items-center border border-black/10 rounded-sm bg-white">
												<button className="px-2 py-1 hover:bg-gray-50 text-gray-600 transition-colors" onClick={() => updateQuantity(item.id, item.quantity - 1)}>
													<Minus size={14} />
												</button>
											...
											<button className="text-xs text-gray-400 hover:text-red-500 transition-colors flex items-center gap-1 uppercase tracking-wider" onClick={() => removeFromCart(item.id)}>
					...
					<div className="lg:col-span-1">
						<div className="bg-white border border-black/5 p-8 rounded-sm sticky top-24 shadow-sm">
							...
							<div className="mb-6 p-4 bg-[#F5F5F2] border border-black/5 text-xs text-gray-500 leading-relaxed rounded-sm">Thoughtful packaging: All orders ship in 100% plastic-free, recyclable materials.</div>

							<Button fullWidth className="bg-[#121212] text-white hover:bg-[#C9A86A] hover:text-[#121212] uppercase tracking-widest text-xs font-bold py-4 transition-all duration-300" onClick={() => navigate("/checkout")}>
