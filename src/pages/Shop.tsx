		...
		<div className="bg-[#F5F5F2] min-h-screen pt-20">
			...
			<div className="relative -mt-20 h-[35vh] bg-[#1C1F26] flex flex-col items-center justify-center overflow-hidden">
				...
				<motion.div className="relative z-10 text-center px-4 flex flex-col items-center" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }}>
					<h1 className="text-6xl md:text-[7rem] lg:text-[9rem] font-serif font-bold text-[#F5F5F2] tracking-tighter leading-none select-none uppercase">PRODUCTS</h1>
				</motion.div>
			</div>
			...
			<div className="max-w-[1440px] mx-auto px-6 md:px-12 pb-32">
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{products.slice(0, 12).map((product, i) => {
						...
						return (
							<Link to={`/product/${product.id}`} key={product.id} className="group relative bg-white h-[420px] rounded-[2rem] overflow-hidden border-b-[6px] border-[#C9A86A] shadow-sm transition-all duration-500 block">
								...
								<div className="absolute top-8 left-8 z-20 flex flex-col items-start gap-3">
									...
									{i < 3 && <span className="bg-red-600 text-white text-[10px] font-bold px-3 py-1.5 uppercase tracking-widest leading-none inline-block">New</span>}

									<span className="text-xs font-bold tracking-[0.2em] uppercase text-gray-400">{product.category}</span>
								</div>

								{/* Main Content Area */}
								<div className="absolute inset-0 p-8 pt-24 z-10 flex flex-col items-start">
									{/* Title - Large & Condensed Style */}
									<h2 className="text-4xl md:text-5xl font-serif font-bold text-[#121212] leading-[0.9] tracking-tighter mb-2 uppercase break-words max-w-[80%]">{product.name.split(" ").slice(0, 2).join(" ")}</h2>
									...
									{/* Learn More Button */}
									<motion.div className="bg-[#121212] text-white px-6 py-3 text-[10px] font-bold tracking-widest uppercase inline-block mt-auto mb-4" whileHover={{ scale: 1.05, backgroundColor: "#C9A86A", color: "#121212" }} transition={{ duration: 0.2 }}>
										Learn More
									</motion.div>
								</div>
								...
								<div className="absolute bottom-[-10%] right-[-10%] w-[70%] h-[70%] z-10 pointer-events-none">
									<motion.img src={productImages[i % productImages.length]} className="w-full h-full object-contain drop-shadow-xl" alt={product.name} whileHover={{ scale: 1.05, rotate: -5 }} transition={{ duration: 0.5 }} />
								</div>
