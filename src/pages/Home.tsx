...
export const Home = () => {
	...
	const topText = useTransform(scrollY, [0, 500], ["10%", isMobile ? "1.5rem" : "1.5rem"]);
	...
	return (
		<div className="bg-[#F5F5F2] overflow-x-hidden text-[#121212]">
			...
			<HeroContainer>
				...
				<HeroContentLayer>
					<div className="flex justify-between items-end w-full">
						...
						<SideContent initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className="max-w-md">
						...
						<SideContent initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="text-right hidden md:block">
							{/* Spinning Text Badge */}
								...
								<motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="absolute inset-0">
									<svg viewBox="0 0 100 100" className="w-full h-full">
										...
										<text fontSize="8.5" letterSpacing="2.2" fill="#121212" fontWeight="500">
											<textPath xlinkHref="#circle">VEGAN • PLASTIC-FREE • MINDFUL •</textPath>
										</text>
									</svg>
								</motion.div>
							...
							<p className="text-xs font-bold tracking-[0.25em] uppercase text-[#121212] mb-4">Est. 2020</p>
							<p className="max-w-[220px] text-sm text-gray-500 leading-relaxed ml-auto mb-6">Designed to support your daily balance with plant-based, plastic-free essentials.</p>
			...
			<BeigeSection>
				<div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
					...
					<div className="relative h-[400px] md:h-[600px] lg:h-[800px]">
						<motion.div className="absolute left-0 top-0 w-3/4 h-3/4 overflow-hidden" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
							<img src={exampleImage} alt="Botanical Texture" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" />
						</motion.div>

						<motion.div className="absolute right-0 bottom-0 w-1/2 h-1/2 bg-white p-4 shadow-xl rotate-3" initial={{ opacity: 0, y: 50, rotate: 0 }} whileInView={{ opacity: 1, y: 0, rotate: 3 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }}>
							<img src={imgOrangeDrink} alt="Orange Drink" className="w-full h-full object-cover" />
						</motion.div>
					</div>
					...
					<div className="space-y-12">
						...
						<FadeIn delay={0.2}>
							<div className="flex gap-4 items-center">
								...
								<p className="max-w-md text-lg text-gray-600 font-light leading-relaxed">Thoughtfully made with zero plastic and vegan ingredients, reflecting our unwavering commitment to your well-being.</p>
							</div>
						</FadeIn>
				...
				<div className="absolute bottom-10 left-10 md:left-20 max-w-xs hidden md:block">
					...
					<p className="text-xs text-gray-500 leading-relaxed">Our commitment to purity means zero compromises. No fillers, no synthetics, just nature's most potent molecules working in harmony.</p>
				</div>
			</BeigeSection>
			...
			{/* --- CATEGORIES (Grid) --- */}
			<section className="flex justify-center align-center w-full py-16">
				<div className="w-full max-w-[1440px] mx-auto px-4 md:px-8 grid grid-cols-1 md:grid-cols-2 gap-4">
					{categories.map((cat, i) => {
						...
						const theme = [
							...
							{ bg: "#121212", text: "#F5F5F2", sub: "text-gray-500", border: "border-black/5" },
						][i % 4];
						return (
							<Link key={cat.id} to={cat.link} className="group relative overflow-hidden block h-[340px] md:h-[380px] rounded-[2rem] transition-all duration-500 shadow-sm" style={{ backgroundColor: theme.bg }}>
								{/* Content Container */}
								<div className="relative z-10 h-full p-8 flex flex-col justify-between">
									...
									<div className="max-w-[60%]">
										<div className="flex items-center gap-3 mb-3">
											...
											<span className={`text-xs font-bold tracking-[0.2em] uppercase ${theme.sub}`}>{cat.subtitle}</span>
										</div>
									...
									<div>
										<div
											className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition-all duration-300 group-hover:bg-opacity-100 ${i % 2 === 0 ? "bg-white/10 group-hover:bg-white text-white group-hover:text-black" : "bg-black/5 group-hover:bg-black text-black group-hover:text-white"}`}>
											<span>Explore</span>
								...
								{/* Dynamic Image Layer */}
								<motion.div className="absolute right-[-2%] bottom-[-5%] w-[55%] h-[80%] z-0" style={{ rotate: cat.rotation }} whileHover={{ scale: 1.1, rotate: 0, x: -10, transition: { type: "spring", stiffness: 300, damping: 20 } }}>
									<img src={cat.image} alt={cat.title} className="w-full h-full object-contain drop-shadow-2xl" />
								</motion.div>
								{/* Decor: Corner Plus */}
